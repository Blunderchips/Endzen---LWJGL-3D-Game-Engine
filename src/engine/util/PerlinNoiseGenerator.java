 ...
 * based on the Improving Noise paper presented at Siggraph 2002.</p>
 * <p>
 * Computing noise for one and two dimensions can make use of the 3D problem
 * space by just setting the un-needed dimensions to a fixed value.</p>
 *
 * @author Justin Couch
...
public class PerlinNoiseGenerator {
     ...
     */
    private static final float LOG_HALF = (float) java.lang.Math.log(0.5);
    ...
    public static void main(String[] args) {
        new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                new PerlinNoiseGenerator(DEFAULT_SEED).display(null);
            }
        }).start();
    }

    public BufferedImage getBufferedImage(int scale, int width, int height) {
        BufferedImage img = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB); // Image to store pixel data in.

        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {

                float xx = (float) x / width * scale; // Where does the point lie in the noise space according to image space. 
                float yy = (float) y / height * scale; // Where does the point lie in the noise space according to image space. 

                float n = (float) this.noise2(xx, yy); // Noise values from Perlin's noise.
                ...
                img.setRGB(x, y, color.getRGB()); // set XY image value to our generated color.
    ...
    public void display(Component parent) {
        JOptionPane.showMessageDialog(parent, null, null, JOptionPane.YES_NO_OPTION,
                new ImageIcon(this.getBufferedImage(16, 512, 512).getScaledInstance(512, 512, Image.SCALE_DEFAULT)));
    }
    ...
    public double improvedNoise(double x, double y, double z) {
        ...
        int uc_x = (int) java.lang.Math.floor(x) & 255;
        int uc_y = (int) java.lang.Math.floor(y) & 255;
        int uc_z = (int) java.lang.Math.floor(z) & 255;

        // Relative location of the point in the unit cube
        double xo = x - java.lang.Math.floor(x);
        double yo = y - java.lang.Math.floor(y);
        double zo = z - java.lang.Math.floor(z);
     ...
     * @param hiF
     */
    public double imporvedTurbulence(double x, double y, double z, float loF,
            float hiF) {
        ...
        for (f = loF; f < hiF; f *= 2) {
            t += java.lang.Math.abs(improvedNoise(p_x, p_y, p_z)) / f;
            p_x *= 2;
            p_y *= 2;
            p_z *= 2;
        }
     ...
     * @return The value at the given coordinates
     */
    public float tileableTurbulence3(float x, float y, float z, float w, float h,
            float d, float freq) {
        float t = 0;
    ...
    private float bias(float a, float b) {
        return (float) java.lang.Math.pow(a, java.lang.Math.log(b) / LOG_HALF);
    }
    ...
    private float gain(float a, float b) {
        ...
        double p = java.lang.Math.log(1.0f - b) / LOG_HALF;

        if (a < 0.5f) {
            return (float) (java.lang.Math.pow(2 * a, p) / 2);
        } else {
            return 1 - (float) (java.lang.Math.pow(2 * (1.0f - a), p) / 2);
        }
    }
    ...
    private void normalize2(float[] v) {
        float s = (float) (1 / java.lang.Math.sqrt(v[0] * v[0] + v[1] * v[1]));
        v[0] *= s;
        v[1] *= s;
    }
    ...
    private void normalize3(float[] v) {
        float s = (float) (1 / java.lang.Math.sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]));
        v[0] *= s;
        v[1] *= s;
        v[2] *= s;
    }
    ...
    private void initPerlin1() {
        ...
        for (i = 0; i < B; i++) {
            ...
            g1[i] = (float) (((java.lang.Math.random() * Integer.MAX_VALUE) % (B + B)) - B) / B;

            for (j = 0; j < 2; j++) {
                g2[i][j] = (float) (((java.lang.Math.random() * Integer.MAX_VALUE) % (B + B)) - B) / B;
            }
            normalize2(g2[i]);

            for (j = 0; j < 3; j++) {
                g3[i][j] = (float) (((java.lang.Math.random() * Integer.MAX_VALUE) % (B + B)) - B) / B;
            }
            normalize3(g3[i]);
        }
        ...
        while (--i > 0) {
            k = p[i];
            j = (int) ((java.lang.Math.random() * Integer.MAX_VALUE) % B);
            p[i] = p[j];
            p[j] = k;
        }
